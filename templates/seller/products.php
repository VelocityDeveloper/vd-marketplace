        ...
        <div class="col-lg-5"><div class="card border-0 shadow-sm"><div class="card-body"><h3 class="h6 mb-2"><?php echo esc_html__('Daftar Produk', 'velocity-marketplace'); ?></h3>
            <?php if ($products_query->have_posts()) : ?>
                ...
                <?php while ($products_query->have_posts()) : $products_query->the_post(); $pid = get_the_ID(); $delete_url = add_query_arg(['tab' => 'seller_products','vmp_delete_product' => $pid,'vmp_nonce' => wp_create_nonce('vmp_delete_product_' . $pid)]); $premium = !empty(get_post_meta($pid, 'premium_request', true)); ?>
                    <tr><td><a href="<?php echo esc_url(get_permalink($pid)); ?>" target="_blank"><?php the_title(); ?></a><?php echo wps_product_price_html($pid, ['wrapper_class' => 'small text-muted mt-1', 'sale_group_class' => 'd-flex align-items-baseline gap-2', 'sale_class' => 'fw-semibold text-dark', 'regular_class' => 'text-muted small', 'price_class' => 'fw-semibold text-dark', 'show_empty' => false]); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><?php if ($premium) : ?><div class="small text-muted"><?php echo esc_html__('Pengajuan premium sedang ditinjau.', 'velocity-marketplace'); ?></div><?php endif; ?></td><td><?php echo esc_html(get_post_status($pid)); ?></td><td class="text-end"><a class="btn btn-outline-dark btn-sm" href="<?php echo esc_url(add_query_arg(['tab' => 'seller_products', 'edit_product' => $pid])); ?>"><?php echo esc_html__('Edit', 'velocity-marketplace'); ?></a> <a class="btn btn-outline-danger btn-sm" href="<?php echo esc_url($delete_url); ?>" onclick="return confirm('<?php echo esc_js(__('Hapus produk ini?', 'velocity-marketplace')); ?>')"><?php echo esc_html__('Hapus', 'velocity-marketplace'); ?></a></td></tr>
