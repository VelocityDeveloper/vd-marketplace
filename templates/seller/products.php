...
$featured_image_url = $featured_image_id > 0
    ? wp_get_attachment_image_url($featured_image_id, 'medium')
    : '';
...
$notice_message = isset($_GET['vmp_notice'])
    ? sanitize_text_field((string) wp_unslash($_GET['vmp_notice']))
    : '';

$error_message = isset($_GET['vmp_error'])
    ? sanitize_text_field((string) wp_unslash($_GET['vmp_error']))
    : '';
?>
...
<div class="row g-3">
    <div class="col-lg-7">
        ...
        <?php if ($error_message !== '' && $field_error_message === '') : ?>
        ...
        <?php if (!$profile_complete) : ?>
            <div class="alert alert-warning">
                <?php
                echo wp_kses_post(
                    __('Lengkapi <strong>Profil Toko</strong> sebelum menambahkan produk baru.', 'velocity-marketplace')
                );
                ?>
            </div>
        <?php endif; ?>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <h3 class="h6 mb-3">
                    ...
                    echo esc_html(
                        $edit_product
                            ? __('Edit Produk', 'velocity-marketplace')
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="row g-2">
                        <div class="col-md-8" data-field-required="1">
                            ...
                            <input
                                type="text"
                                id="vmp_product_title"
                                name="title"
                                class="form-control<?php echo $field_error === 'title' ? ' is-invalid' : ''; ?>"
                                data-required="1"
                                value="<?php echo esc_attr($defaults['title']); ?>"
                            >

                            <?php if ($field_error === 'title' && $field_error_message !== '') : ?>
                                <div class="invalid-feedback d-block">
                                    <?php echo esc_html($field_error_message); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="category_id" id="vmp_category_id" class="form-select">
                                <option value="0">
                                    <?php echo esc_html__('Pilih kategori', 'velocity-marketplace'); ?>
                                </option>
                        ...
                        <div class="col-12">
                            <label class="form-label">
                                <?php echo esc_html__('Deskripsi', 'velocity-marketplace'); ?>
                            </label>
                            ...
                            <div class="vmp-editor-wrap">
                                ...
                                wp_editor(
                                    ...
                                    'vmp_product_description_' . ($edit_product ? $edit_product_id : 'new'),
                                    [
                        ...
                        <div class="col-12">
                            <label class="form-label">
                                <?php echo esc_html__('Gambar Utama', 'velocity-marketplace'); ?>
                            </label>
                            ...
                            <div class="vmp-media-field" data-multiple="0" data-overlay-remove="0">
                                ...
                                <div class="d-flex flex-wrap gap-2 mb-2">
                                    <button
                                        type="button"
                                        class="btn btn-outline-dark btn-sm vmp-media-field__open"
                                        data-title="<?php echo esc_attr__('Gambar Utama', 'velocity-marketplace'); ?>"
                                        data-button="<?php echo esc_attr__('Gunakan gambar ini', 'velocity-marketplace'); ?>"
                                    >
                                        <?php echo esc_html__('Pilih dari Media Library', 'velocity-marketplace'); ?>
                                    </button>

                                    <button
                                        type="button"
                                        class="btn btn-outline-secondary btn-sm vmp-media-field__clear"
                                        <?php disabled($featured_image_id <= 0); ?>
                                    >
                                        <?php echo esc_html__('Remove Image', 'velocity-marketplace'); ?>
                                    </button>
                                </div>

                                <div
                                    class="vmp-media-field__preview"
                                    data-placeholder="<?php echo esc_attr__('No featured image yet.', 'velocity-marketplace'); ?>"
                                >
                                    <?php if ($featured_image_url) : ?>
                                        <div class="vmp-media-field__grid vmp-media-field__grid--single">
                                            <div
                                                class="vmp-media-field__item"
                                                data-id="<?php echo esc_attr((string) $featured_image_id); ?>"
                                            >
                                                <img
                                                    src="<?php echo esc_url($featured_image_url); ?>"
                                                    alt="<?php echo esc_attr__('Featured product image', 'velocity-marketplace'); ?>"
                                                    class="vmp-media-field__image"
                                                >
                                            </div>
                                        </div>
                                    <?php else : ?>
                                        <div class="vmp-media-field__empty text-muted small">
                                            <?php echo esc_html__('No image selected.', 'velocity-marketplace'); ?>
                                        </div>
                                    <?php endif; ?>
                                </div>

                                <div class="form-text">
                                    <?php echo esc_html__('Pilih atau unggah gambar utama melalui media library WordPress.', 'velocity-marketplace'); ?>
                                </div>
                            </div>
                        </div>

                        <?php
                        echo ProductFields::render_sections(
                            $edit_product ? $edit_product_id : 0,
                            'frontend'
                        ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                        echo '<div class="col-12">';
                            echo vmp_premium_badge_html([
                                'post_id' => $edit_product_id,
                                'text' => 'Produk Premium',
                            ]);
                        echo '</div>';
                    ...
                    <?php if ($product_captcha_html !== '') : ?>
                        <div class="mt-3">
                            <?php echo $product_captcha_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="mt-3 d-flex gap-2">
                        <button
                            ...
                            type="submit"
                            <?php disabled(!$profile_complete); ?>
                        >
                            <?php
                            echo esc_html(
                                $edit_product
                                    ? __('Save Changes', 'velocity-marketplace')
                                    : __('Simpan Produk', 'velocity-marketplace')
                            );
                            ?>
                        </button>
                                ...
                                <?php while ($products_query->have_posts()) : ?>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <?php if (vmp_is_premium_product($pid)):
                                                echo vmp_premium_badge_html();
                                            elseif ($premium) : ?>
