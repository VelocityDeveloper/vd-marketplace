                        ...
                        <?php foreach ($message_contacts as $contact) : ?>
                            ...
                            <a href="<?php echo esc_url($thread_url); ?>" class="list-group-item list-group-item-action border mb-2 px-2<?php echo $is_active ? ' active' : ''; ?>">
                                <div class="d-flex justify-content-between align-items-start gap-2">
                                    <div class="min-w-0">
                                        <div class="fw-semibold"><?php echo esc_html((string) ($contact['name'] ?? __('Pengguna', 'velocity-marketplace'))); ?></div>
                                        ...
                                        <?php if (!empty($contact['last_order_invoice'])) : ?>
                                            <div class="small mt-1">
                                                <span class="badge <?php echo $is_active ? 'bg-light text-dark' : 'bg-light text-dark border'; ?>">
                                                    <?php echo esc_html(sprintf(__('Invoice: %s', 'velocity-marketplace'), (string) $contact['last_order_invoice'])); ?>
                                                </span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="text-end">
                                        <?php if (!empty($contact['unread_count'])) : ?>
                                            <span class="badge bg-danger"><?php echo esc_html(sprintf(__('Belum Dibaca %d', 'velocity-marketplace'), (int) $contact['unread_count'])); ?></span>
                                        <?php endif; ?>
            ...
            <div class="card-body">
                ...
                <?php else : ?>
                    <div class="d-flex justify-content-between align-items-center gap-2 mb-3 flex-wrap">
                        <div>
                            <h3 class="h6 mb-0"><?php echo esc_html((string) ($selected_message_contact['name'] ?? __('Pengguna', 'velocity-marketplace'))); ?></h3>
                            <div class="small text-muted">
                                <?php echo esc_html((string) ($selected_message_contact['role'] ?? '')); ?>
                                <?php if ($selected_message_invoice !== '') : ?>
                                    <span> | </span>
                                    <span><?php echo esc_html(sprintf(__('Invoice: %s', 'velocity-marketplace'), $selected_message_invoice)); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="d-flex <?php echo !empty($row['incoming']) ? 'justify-content-start' : 'justify-content-end'; ?>">
                                        <div class="border rounded px-3 py-2 bg-white" style="max-width:82%;">
                                            <div class="small fw-semibold mb-1">
                                                <?php echo esc_html(!empty($row['incoming']) ? (string) ($row['partner_name'] ?? __('Pengguna', 'velocity-marketplace')) : __('Saya', 'velocity-marketplace')); ?>
                                            </div>
                                            ...
                                            <div class="small text-muted mt-2">
                                                ...
                                                <?php if (!empty($row['order_invoice'])) : ?>
                                                    <span> | </span>
                                                    <span><?php echo esc_html(sprintf(__('Invoice: %s', 'velocity-marketplace'), (string) $row['order_invoice'])); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="border rounded p-3">
                        ...
                        <form method="post" class="row g-2">
                            ...
                            <?php if ($selected_message_invoice !== '') : ?>
                                <div class="col-12">
                                    <div class="form-text mt-0"><?php echo esc_html(sprintf(__('Percakapan ini terhubung ke invoice %s.', 'velocity-marketplace'), $selected_message_invoice)); ?></div>
                                </div>
                            <?php endif; ?>
                            <div class="col-12">
                                <textarea name="message" class="form-control" rows="5" placeholder="<?php echo esc_attr__('Tulis pesan Anda', 'velocity-marketplace'); ?>" required></textarea>
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-dark"><?php echo esc_html__('Kirim Pesan', 'velocity-marketplace'); ?></button>
                            </div>
                        </form>
                    </div>
                <?php endif; ?>
