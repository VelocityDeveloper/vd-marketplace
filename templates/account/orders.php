...
use VelocityMarketplace\Modules\Shipping\ShippingController;
    ...
    <?php elseif ($invoice !== '') : ?>
        ...
        $coupon_code = (string) get_post_meta($order_id, '_store_order_coupon_code', true);
        $coupon_scope = (string) get_post_meta($order_id, '_store_order_coupon_scope', true);
        $coupon_discount = (float) get_post_meta($order_id, '_store_order_discount_amount', true);
        $coupon_product_discount = (float) get_post_meta($order_id, '_store_order_coupon_product_discount', true);
        $coupon_shipping_discount = (float) get_post_meta($order_id, '_store_order_coupon_shipping_discount', true);
    ...
    <?php else : ?>
        ...
        <?php while ($query->have_posts()) : $query->the_post(); $order_id = get_the_ID(); $invoice_meta = (string) get_post_meta($order_id, 'vmp_invoice', true); $status = (string) get_post_meta($order_id, 'vmp_status', true); $total = (float) get_post_meta($order_id, 'vmp_total', true); ?>
            <tr><td><?php echo esc_html($invoice_meta); ?></td><td><?php echo esc_html(get_the_date('d-m-Y H:i', $order_id)); ?></td><td><?php echo esc_html(OrderData::status_label($status)); ?></td><td class="text-end"><?php echo esc_html(number_format($total, 0, ',', '.')); ?></td><td class="text-end"><a class="btn btn-sm btn-outline-dark" href="<?php echo esc_url(\VelocityMarketplace\Support\Settings::customer_order_url($invoice_meta)); ?>"><?php echo esc_html__('Lihat Detail', 'velocity-marketplace'); ?></a></td></tr>
